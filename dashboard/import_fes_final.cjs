...
const FES_DATA = {
  ...
  missions: [
    {
      id: 'F1', title: 'L’Atelier de calligraphie', skill: 'Travail en équipe (analyse)',
      questions: [
...
async function importFes() {
  console.log('🚀 Starting Fès CLEAN import...')
  
  // Cleanup duplicates first (rows with city_id 'fes')
  ...
  else console.log('✅ City Fès upserted with ID:', FES_CHALLENGE_PK)
  ...
  for (const mission of FES_DATA.missions) {
    ...
    const { error: missErr } = await supabase
      ...
      .upsert({
        id: missionId,
        city_id: FES_CHALLENGE_PK, // Foreign key to challenges.id
        title_fr: mission.title,
        sort_order: parseInt(mission.id.substring(1)),
        challenge_id: FES_CHALLENGE_PK // Backward compatibility
      })
    ...
    const questionsToInsert = mission.questions.map((q, i) => {
      ...
      return {
        ...
        question_fr: q.q,
        question_type: TYPE_MAPPING[q.type] || q.type,
        options: optionsData,
