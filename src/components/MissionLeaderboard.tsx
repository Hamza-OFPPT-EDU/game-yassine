  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-[11px] font-black uppercase tracking-[0.4em] text-voyage-primary/50">Calcul des scores...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 bg-voyage-accent/10 px-4 py-1 rounded-full border border-voyage-accent/20 mb-2">
           ...
           <span className="text-[10px] font-black uppercase tracking-widest text-voyage-primary">Classement Mondial</span>
        </div>
        <h2 className="text-3xl font-black text-voyage-primary-dark">Tableau des Champions</h2>
      </div>

      <div className="bg-white/40 backdrop-blur-md rounded-[2.5rem] border border-white/60 shadow-xl overflow-hidden">
        <div className="divide-y divide-voyage-secondary/30 max-h-[600px] overflow-y-auto custom-scrollbar">
          {leaderboard.map((player, index) => {
            ...
            return (
              ...
              >
                <div className="w-10 text-center font-black text-lg text-voyage-primary/40">
                  {isTop3 ? (
                    <div className={cn(
                      "w-10 h-10 rounded-xl flex items-center justify-center mx-auto",
                      index === 0 ? "bg-voyage-accent text-white shadow-lg shadow-voyage-accent/20" :
                      index === 1 ? "bg-slate-300 text-white shadow-lg shadow-slate-200" :
                      "bg-voyage-primary text-white shadow-lg shadow-voyage-primary/20"
                    )}>
                      <Trophy size={18} />
                    </div>
                  ) : (
                ...
                <div className="grow">
                  <div className="flex items-center gap-2">
                    <h4 className="font-black text-voyage-primary text-lg">{player.name}</h4>
                    {isMe && (
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-bold text-voyage-secondary-dark uppercase tracking-widest">Niveau {player.level}</span>
                    <div className="h-1 w-1 rounded-full bg-voyage-secondary/50" />
                ...
                <div className="text-right">
                   <div className="flex items-center gap-1.5 justify-end">
                      ...
                      <span className="font-black text-2xl text-voyage-primary-dark">{player.score.toLocaleString()}</span>
                   </div>
                   <p className="text-[10px] font-black text-voyage-primary/40 uppercase tracking-widest">Points XP</p>
                </div>
              </motion.div>
            );
          })}
      ...
      <motion.div 
        ...
        className="bg-linear-to-br from-voyage-sand/30 to-voyage-parchment/10 border-2 border-voyage-secondary/30 p-8 rounded-4xl text-center space-y-4 shadow-xl relative overflow-hidden"
      >
        <div className="relative z-10">
          <PartyPopper size={48} className="text-voyage-accent mx-auto mb-4" />
          <h3 className="text-2xl font-black text-voyage-primary italic">"Chaque étape est une victoire sur soi-même !"</h3>
          <p className="text-voyage-primary/60 text-sm font-bold uppercase tracking-widest">Continue ton voyage, l'aventure ne fait que commencer.</p>
        </div>
